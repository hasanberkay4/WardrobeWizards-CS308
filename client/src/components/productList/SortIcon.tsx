...
// takes function as a prop
function SortingMenu({ onSortOptionSelected }: SortingMenuProps) {
    const [selectedSort, setSelectedSort] = useState(sortOptions[0].name);

    const handleSortOptionClick = (name: string) => {
                                ...
                                <Menu.Item key={option.name}>
                                    {({ active }) => (
                                        <a
                                            ...
                                            className={`${option.name === selectedSort ? 'font-medium text-gray-900' : 'text-gray-500'
