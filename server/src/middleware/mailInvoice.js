...
const nodemailer = require('nodemailer');
const pdfkit = require('pdfkit');
const fs = require('fs'); // Importing the `promises` version of `fs` to use async/await with `fs`
const logo = '../../../client/public/logo.jpg';
const signature = '../../../client/public/signature.png';
...
const sendInvoiceEmail = async (invoice) => {
  ...
  delivery.products.forEach((item) => {
    doc.text(`${item.name}: $${item.price.toFixed(2)}`, { align: 'left' });
  });
  doc.moveDown();
  doc.text(`Total: $${delivery.totalPrice}`, { align: 'right' });
  doc.image(signature, {
    width: 100,
    height: 100,
    align: 'right'
